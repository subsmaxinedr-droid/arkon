...
var init_errors = __esm({
  "src/errors.ts"() {
    ...
    InvoiceExpiredError = class extends SwapError {
      constructor(options = {}) {
        super({ message: "The invoice has expired.", ...options });
        this.name = "InvoiceExpiredError";
      }
    };
    InvoiceFailedToPayError = class extends SwapError {
      constructor(options = {}) {
        super({
...
var init_boltz_swap_provider = __esm({
  "src/boltz-swap-provider.ts"() {
    "use strict";
    import_sdk = require("@arkade-os/sdk");
    init_errors();
    ...
    isGetReverseSwapTxIdResponse = (data) => {
    ...
    isGetReversePairsResponse = (data) => {
    ...
    isRefundChainSwapResponse = (data) => {
      return data && typeof data === "object" && typeof data.transaction === "string" && typeof data.checkpoint === "string";
    };
    isGetChainPairsResponse = (data) => {
    ...
    isChainSwapDetailsResponse = (data) => {
    ...
    isCreateChainSwapResponse = (data) => {
    ...
    isGetChainClaimDetailsResponse = (data) => {
    ...
    isPostChainClaimDetailsResponse = (data) => {
    ...
    isGetChainQuoteResponse = (data) => {
      return data && typeof data === "object" && typeof data.amount === "number";
    };
    isPostChainQuoteResponse = (data) => {
      return data && typeof data === "object" && Object.keys(data).length === 0 && data.constructor === Object;
    };
    isPostBtcTransactionResponse = (data) => {
...
// src/utils/musig.ts
var import_secp256k1, import_base2, import_musig2, import_utils, findKeyIndex, assertPublicKeys, aggregateKeys, MusigKeyAgg, MusigWithMessage, MusigWithNonce, MusigNoncesAggregated, MusigSession, MusigSigned, create;
var init_musig = __esm({
  "src/utils/musig.ts"() {
    "use strict";
    import_secp256k1 = require("@noble/curves/secp256k1.js");
    import_base2 = require("@scure/base");
    import_musig2 = require("@scure/btc-signer/musig2.js");
    import_utils = require("@scure/btc-signer/utils.js");
    findKeyIndex = (keys, target) => keys.findIndex((k) => (0, import_utils.equalBytes)(target, k));
    ...
    aggregateKeys = (publicKeys, tweak) => {
    ...
    MusigKeyAgg = class _MusigKeyAgg {
    ...
    MusigNoncesAggregated = class {
      ...
      initializeSession() {
        const session = new import_musig2.Session(
    ...
    create = (privateKey, publicKeys) => {
      if (publicKeys.length < 2)
        throw new Error("need at least 2 keys to aggregate");
      const keys = [...publicKeys];
      assertPublicKeys(keys);
      Object.freeze(keys);
      const myPublicKey = import_secp256k1.secp256k1.getPublicKey(privateKey);
...
var import_secp256k12, import_base3, import_btc_signer, import_payment, import_utils2, REGTEST_NETWORK, MUTINYNET_NETWORK, deserializeLeaf, sortTree, deserializeSwapTree, taprootHashTree, tweakMusig, toXOnly, p2trScript, detectSwapOutput, DUMMY_TAPROOT_SIGNATURE, constructClaimTransaction, targetFee;
var init_boltz_swap_tx = __esm({
  "src/utils/boltz-swap-tx.ts"() {
    "use strict";
    import_secp256k12 = require("@noble/curves/secp256k1.js");
    import_base3 = require("@scure/base");
    import_btc_signer = require("@scure/btc-signer");
    import_payment = require("@scure/btc-signer/payment.js");
    import_utils2 = require("@scure/btc-signer/utils.js");
    REGTEST_NETWORK = {
      bech32: "bcrt",
      pubKeyHash: 111,
      scriptHash: 196,
    ...
    taprootHashTree = (tree) => {
      if (!Array.isArray(tree)) {
        return {
          ...
          script: tree.output,
          hash: (0, import_payment.tapLeafHash)(tree.output, tree.version)
        };
      }
      const left = taprootHashTree(tree[0]);
      const right = taprootHashTree(tree[1]);
      let [lH, rH] = [left.hash, right.hash];
      if ((0, import_utils2.compareBytes)(rH, lH) === -1) [lH, rH] = [rH, lH];
      return {
        type: "branch",
        left,
        right,
        hash: import_secp256k12.schnorr.utils.taggedHash("TapBranch", lH, rH)
      };
    };
    tweakMusig = (musig, tree) => {
    ...
    p2trScript = (publicKey) => import_btc_signer.Script.encode(["OP_1", toXOnly(publicKey)]);
    detectSwapOutput = (tweakedKey, transaction) => {
      ...
      for (let vout = 0; vout < transaction.outputsLength; vout++) {
        ...
        if (output.script !== void 0 && output.amount !== void 0 && (0, import_utils2.equalBytes)(target, output.script)) {
          return { ...output, vout };
        }
      }
      throw new Error("Swap output not found in transaction");
    };
    DUMMY_TAPROOT_SIGNATURE = new Uint8Array(64);
    constructClaimTransaction = (utxo, destinationScript, fee) => {
    ...
    targetFee = (satPerVbyte, constructTx) => {
...
var import_light_bolt11_decoder, import_sdk2, decodeInvoice, getInvoicePaymentHash;
var init_decoding = __esm({
  "src/utils/decoding.ts"() {
    "use strict";
    import_light_bolt11_decoder = __toESM(require("light-bolt11-decoder"), 1);
    import_sdk2 = require("@arkade-os/sdk");
...
var import_sdk3, import_base4, verifySignatures, normalizeToXOnlyKey;
var init_signatures = __esm({
  "src/utils/signatures.ts"() {
    ...
    normalizeToXOnlyKey = (someKey, keyName = "", swapId = "") => {
      const keyBytes = typeof someKey === "string" ? import_base4.hex.decode(someKey) : someKey;
...
function extractTimeLockFromLeafOutput(scriptHex) {
  ...
  try {
    const opcodes = import_btc_signer2.Script.decode(import_base5.hex.decode(scriptHex));
    ...
    if (hasCLTV > 0) {
      ...
      if (data instanceof Uint8Array) {
        ...
        return parseInt(import_base5.hex.encode(dataBytes), 16);
      }
    }
...
var import_base5, import_btc_signer2, import_bip68;
var init_restoration = __esm({
  "src/utils/restoration.ts"() {
    "use strict";
    import_base5 = require("@scure/base");
    import_btc_signer2 = require("@scure/btc-signer");
    import_bip68 = __toESM(require("bip68"), 1);
  }
});
...
function enrichReverseSwapPreimage(swap, preimage) {
  const computedHash = import_base6.hex.encode((0, import_sha2.sha256)(import_base6.hex.decode(preimage)));
...
var import_sha2, import_base6;
var init_swap_helpers = __esm({
  "src/utils/swap-helpers.ts"() {
    "use strict";
    import_sha2 = require("@noble/hashes/sha2.js");
    import_base6 = require("@scure/base");
    init_boltz_swap_provider();
    init_decoding();
  }
});
...
var import_sdk4, DEFAULT_DB_NAME, DB_VERSION, STORE_SWAPS_STATE, IndexedDbSwapRepository, FILTER_FIELDS;
var init_swap_repository = __esm({
  "src/repositories/IndexedDb/swap-repository.ts"() {
    "use strict";
    import_sdk4 = require("@arkade-os/sdk");
    DEFAULT_DB_NAME = "arkade-boltz-swap";
    DB_VERSION = 2;
    STORE_SWAPS_STATE = "swaps";
    IndexedDbSwapRepository = class {
      ...
      async getDB() {
        ...
        this.db = await (0, import_sdk4.openDatabase)(this.dbName, DB_VERSION, initDatabase);
        return this.db;
      }
      async saveSwap(swap) {
...
var init_identity = __esm({
  "src/utils/identity.ts"() {
    "use strict";
    import_sdk5 = require("@arkade-os/sdk");
  }
});
...
function createVHTLCBatchHandler(intentId, vhtlc, arkProvider, identity, session, sweepPublicKey, forfeitOutputScript, connectorIndex = 0) {
  ...
  const intentIdHash = (0, import_sha22.sha256)(utf8IntentId);
  const intentIdHashStr = import_base7.hex.encode(intentIdHash);
  ...
  return {
    onBatchStarted: async (event) => {
      ...
      const sweepTapscript = import_sdk6.CSVMultisigTapscript.encode({
        timelock: {
      ...
      sweepTapTreeRoot = (0, import_payment2.tapLeafHash)(sweepTapscript);
      return { skip: false };
    },
...
function createForfeitTx(input, forfeitOutputScript, connector) {
  ...
  const sequence = (0, import_sdk6.getSequence)(input.tapLeafScript);
  return (0, import_sdk6.buildForfeitTx)(
    [
      {
        txid: input.txid,
        index: input.vout,
        witnessUtxo: {
          ...
          script: import_sdk6.VtxoScript.decode(input.tapTree).pkScript
        },
        sighashType: import_btc_signer3.SigHash.DEFAULT,
...
var import_sdk6, import_sha22, import_base7, import_btc_signer3, import_payment2;
var init_batch = __esm({
  "src/batch.ts"() {
    "use strict";
    import_sdk6 = require("@arkade-os/sdk");
    import_sha22 = require("@noble/hashes/sha2.js");
    import_base7 = require("@scure/base");
    import_btc_signer3 = require("@scure/btc-signer");
    import_payment2 = require("@scure/btc-signer/payment.js");
  }
});
...
var init_vhtlc = __esm({
  "src/utils/vhtlc.ts"() {
    ...
    joinBatch = async (arkProvider, identity, input, output, {
      forfeitPubkey,
      forfeitAddress,
      network
    }, isRecoverable2 = true) => {
      ...
      const intentMessage = {
        ...
        valid_at: 0,
        expire_at: 0,
        cosigners_public_keys: [import_base8.hex.encode(signerPublicKey)]
      };
      const deleteMessage = {
        type: "delete",
        expire_at: 0
      ...
      const intentInput = {
        txid: import_base8.hex.decode(input.txid),
        index: input.vout,
        witnessUtxo: {
          amount: BigInt(input.value),
          script: import_sdk7.VtxoScript.decode(input.tapTree).pkScript
        },
        tapLeafScript: [input.tapLeafScript],
      ...
      const intentId = await arkProvider.registerIntent({
        ...
        proof: import_base8.base64.encode(signedRegisterIntent.toPSBT())
      });
      const decodedAddress = (0, import_btc_signer4.Address)(
        network in import_sdk7.networks ? import_sdk7.networks[network] : import_sdk7.networks.bitcoin
      ...
      try {
        ...
        try {
          await arkProvider.deleteIntent({
            ...
            proof: import_base8.base64.encode(signedDeleteIntent.toPSBT())
          });
        } catch (error2) {
    ...
    claimVHTLCwithOffchainTx = async (identity, vhtlcScript, serverXOnlyPublicKey, input, output, arkInfo, arkProvider) => {
    ...
    refundVHTLCwithOffchainTx = async (swapId, identity, arkProvider, boltzXOnlyPublicKey, ourXOnlyPublicKey, serverXOnlyPublicKey, input, output, arkInfo, refundFunc) => {
      const rawCheckpointTapscript = import_base8.hex.decode(arkInfo.checkpointTapscript);
      const serverUnrollScript = import_sdk7.CSVMultisigTapscript.decode(
        rawCheckpointTapscript
      );
...
var init_arkade_swaps = __esm({
  "src/arkade-swaps.ts"() {
    "use strict";
    init_errors();
    import_sdk8 = require("@arkade-os/sdk");
    init_boltz_swap_provider();
    import_sha23 = require("@noble/hashes/sha2.js");
    import_base9 = require("@scure/base");
    import_secp256k13 = require("@noble/curves/secp256k1.js");
    import_utils3 = require("@noble/hashes/utils.js");
    import_btc_signer5 = require("@scure/btc-signer");
    import_utils4 = require("@scure/btc-signer/utils.js");
    init_musig();
    init_boltz_swap_tx();
    init_decoding();
    init_signatures();
    ...
    ArkadeSwaps = class {
      ...
      async createReverseSwap(args) {
        ...
        const pendingSwap = {
          ...
          preimage: import_base9.hex.encode(preimage),
          request: swapRequest,
      ...
      async claimBtc(pendingSwap) {
        ...
        const musig = tweakMusig(
          create(import_base9.hex.decode(pendingSwap.ephemeralKey), [
            ...
            import_secp256k13.secp256k1.getPublicKey(import_base9.hex.decode(pendingSwap.ephemeralKey))
          ]),
          swapTree.tree
        );
        ...
        const signedTxData = await this.swapProvider.postChainClaimDetails(
          pendingSwap.response.id,
          {
            preimage: pendingSwap.preimage,
            toSign: {
              pubNonce: import_base9.hex.encode(musigMessage.publicNonce),
        ...
        musigSession.addPartial(
          import_base9.hex.decode(pendingSwap.response.claimDetails.serverPublicKey),
          import_base9.hex.decode(signedTxData.partialSignature)
        );
      ...
      async signCooperativeClaimForServer(pendingSwap) {
        ...
        await this.swapProvider.postChainClaimDetails(pendingSwap.response.id, {
          signature: {
            partialSignature: import_base9.hex.encode(partialSig.ourPartialSignature),
            pubNonce: import_base9.hex.encode(partialSig.publicNonce)
          }
        });
      }
      /**
      ...
      async createChainSwap(args) {
        ...
        const claimPublicKey = to === "ARK" ? import_base9.hex.encode(await this.wallet.identity.compressedPublicKey()) : import_base9.hex.encode(import_secp256k13.secp256k1.getPublicKey(ephemeralKey));
        if (!claimPublicKey)
          throw new SwapError({
        ...
        const pendingSwap = {
          amount,
          createdAt: Math.floor(Date.now() / 1e3),
          ephemeralKey: import_base9.hex.encode(ephemeralKey),
          feeSatsPerByte,
          id: swapResponse.id,
          preimage: import_base9.hex.encode(preimage),
          request: swapRequest,
...
var SWAP_POLL_TASK_TYPE, swapsPollProcessor;
var init_swapsPollProcessor = __esm({
  "src/expo/swapsPollProcessor.ts"() {
    "use strict";
    init_boltz_swap_provider();
    init_arkade_swaps();
    init_logger();
    SWAP_POLL_TASK_TYPE = "swap-poll";
    swapsPollProcessor = {
...
__export(background_exports, {
  defineExpoSwapBackgroundTask: () => defineExpoSwapBackgroundTask,
  registerExpoSwapBackgroundTask: () => registerExpoSwapBackgroundTask,
  unregisterExpoSwapBackgroundTask: () => unregisterExpoSwapBackgroundTask
});
...
var import_expo, import_expo2;
var init_background = __esm({
  "src/expo/background.ts"() {
    "use strict";
    import_expo = require("@arkade-os/sdk/worker/expo");
    import_expo2 = require("@arkade-os/sdk/adapters/expo");
    init_boltz_swap_provider();
    init_swapsPollProcessor();
  }
});
...
__export(expo_exports, {
  ExpoArkadeLightning: () => ExpoArkadeLightning,
  ExpoArkadeSwaps: () => ExpoArkadeSwaps,
  SWAP_POLL_TASK_TYPE: () => SWAP_POLL_TASK_TYPE,
  defineExpoSwapBackgroundTask: () => defineExpoSwapBackgroundTask,
  registerExpoSwapBackgroundTask: () => registerExpoSwapBackgroundTask,
  swapsPollProcessor: () => swapsPollProcessor,
  unregisterExpoSwapBackgroundTask: () => unregisterExpoSwapBackgroundTask
});
...
var ExpoArkadeSwaps = class _ExpoArkadeSwaps {
  ...
  static async setup(config) {
    ...
    if (config.background.minimumBackgroundInterval) {
      try {
        const { registerExpoSwapBackgroundTask: registerExpoSwapBackgroundTask2 } = await Promise.resolve().then(() => (init_background(), background_exports));
  ...
  async dispose() {
    ...
    try {
      const { unregisterExpoSwapBackgroundTask: unregisterExpoSwapBackgroundTask2 } = await Promise.resolve().then(() => (init_background(), background_exports));
